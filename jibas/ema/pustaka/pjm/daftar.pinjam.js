...
function acceptPegawai(noanggota,nama,flag){
	var kriteria = document.getElementById('kriteria').value;
	document.location.href="../pustaka/pjm/daftar.pinjam.php?kriteria="+kriteria+"&noanggota="+noanggota+"&nama="+nama;
}
function TakeDate(elementid){
	var addr = "../../lib/cals.php?elementid="+elementid;
	newWindow(addr, 'CariTanggal','376','240','resizable=0,scrollbars=0,status=0,toolbar=0')
}
function AcceptDate(date,elementid){
	...
	var tglAwal = document.getElementById('tglAwal').value;
	var tglAkhir = document.getElementById('tglAkhir').value;
	document.location.href="../pustaka/pjm/daftar.pinjam.php?kriteria="+kriteria+"&tglAwal="+tglAwal+"&tglAkhir="+tglAkhir;
}